...
use App\Services\ProductionSupplierCreateService;
use App\Services\ProductionSupplierUpdateService;
...
class ProductionSupplierController extends Controller
{
	...
	public function index()
	{
		try {
			...
			$suppliers = ProductionSupplier::orderBy('id', 'desc');
			if ($id) {
			...
			if ($category_id) {
				$suppliers = $suppliers->whereIn('id', function ($query) use ($category_id) {
					$query->select('supplier_id')
						->from(DBHelper::TB_PRODUCTION_SUPPLIER_PRODUCTS)
	...
	public function store(SupplierCreateRequest $request, ProductionSupplierCreateService $service)
	{
		try {
	...
	public function update(SupplierUpdateRequest $request, ProductionSupplierUpdateService $service, ProductionSupplier $supplier)
	{
		try {
	...
	{
		try {
			return HttpResponse::success(data: ProductionSupplier::count());
		} catch (\Throwable $th) {
			return ErrorHandler::handle(exception: $th, message: 'Erro ao consultar fornecedor');
		}
	}

	public function destroy(ProductionSupplier $supplier)
	{
		try {
			$supplier->delete();
